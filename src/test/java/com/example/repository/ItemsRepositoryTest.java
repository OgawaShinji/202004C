...
public class ItemsRepositoryTest {
	 ...
	 * FindAllメソッドのテスト
	 */
	// 価格が安い順(サイズM)の並び順が0番目の時のテスト
	@Test
	public void orderBySizeM() {
		...
		assertEquals("解説が違います", item.getDescription(),
	...
	@Test
	// 価格が安い順(サイズM)の並び順が1番目の時のテスト
	public void orderBySizeM2() {
	...
	@Test
	// 価格が高い順(サイズM)の並び順が0番目の時のテスト
	public void orderBySizeMDesc() {
	...
	public void orderBySizeL2() {
		...
		assertEquals("解説が違います", item.getDescription(),
				"キャラメルのような甘く力強い味とナッツを感じさせる後味。スターバックスのすべてのエスプレッソ ビバレッジに使われているエスプレッソです。どうぞ、お早めにお召し上がりください。");
		assertEquals("Mの価格が違います", item.getPriceM(), priceM);
		assertEquals("Lの価格が違います", item.getPriceL(), priceL);
	...
	public void orderByName() {
		Item item = itemsRepository.findAll("name").get(0);
		Integer id = 1;
		Integer priceM = 480;
		Integer priceL = 700;
		Integer categoryId = 2;
	...
	@Test
	// 名前順の並び順が1番目の時のテスト
	public void orderByname2() {
		Item item = itemsRepository.findAll("name").get(1);
		Integer id = 3;
		Integer priceM = 250;
		Integer priceL = 500;
		Integer categoryId = 2;
